 ...
 n_layers, lags, n_features, breg, kreg, rreg, lr, lrd, do):
    ...
    if model == None:
        ...
        for i in range(n_layers-1):   
            model.add(LSTM(n_neurons, activation = None, bias_regularizer=breg, kernel_regularizer=kreg, recurrent_regularizer=rreg, recurrent_dropout=0.0, return_sequences=True))
            model.add(BatchNormalization()) 
            model.add(layers.Activation('tanh'))
            model.add(Dropout(do))      
            n_neurons = int(n_neurons*neuron_decay_factor_per_layer)
            
        model.add(LSTM(n_neurons, activation = None, bias_regularizer=breg, kernel_regularizer=kreg, recurrent_regularizer=rreg, recurrent_dropout=0.0, return_sequences=False))
        model.add(BatchNormalization()) 
        model.add(layers.Activation('tanh'))
        model.add(Dropout(do)) 
